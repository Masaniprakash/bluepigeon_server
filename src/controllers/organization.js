...
exports.createOrganization = async (req, res) => {
    ...
    [err, updateOrgUser] = await to(
        user_data.update({ organization_ids: [...user.organization_ids, org.id] }, { where: { id: user.id } })
    );
    if (err) return ReE(res, err, httpStatus.INTERNAL_SERVER_ERROR);
...
exports.updateOrganization = async (req, res) => {
    ...
    if (body.name) {
        let existingOrg;
        [err, existingOrg] = await to(organization.findOne({ where: { name: body.name, id: { [db.Sequelize.Op.ne]: orgId } } }));
        if (err) return ReE(res, err, httpStatus.INTERNAL_SERVER_ERROR);
        if (existingOrg) return ReE(res, { message: "Organization name already exists" }, httpStatus.BAD_REQUEST);
    }
